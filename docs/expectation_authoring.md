...
`agent-run-record-producer-contract/v2`: declare structured field origins, emit
explicit `claim_evidence_links` for every material claim they intend to
satisfy, and point each link to a present `evidence_items[].ref_id` in the same
run record.
...
the declared review route and record `human_review_performed=true`. A route flag
without performed review fails the deterministic human-review control.
