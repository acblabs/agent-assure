...
Release hardening adds frozen-schema parity to `make release-check`, verifies
every frozen schema file in the built and installed wheel, pins remaining
GitHub Actions references to reviewed SHAs, requires explicit TestPyPI version
input, isolates reusable-action shell commands from caller-provided input
strings, asserts that core commands ignore demo-only environment markers, checks
the installed demo network guard in wheel smoke tests, and moves release-bundle
cosign signing and verification artifact selection out of workflow shell
snippets into a Python helper.

## Release status

Published to PyPI and post-publish validated on 2026-07-02. The installed
`agent-assure==0.3.0` package ran the packaged `agent-assure demo flagship`
flow successfully, preserving output equivalence while blocking the expected
process-assurance regression.
