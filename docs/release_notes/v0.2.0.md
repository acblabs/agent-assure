...
non-default OpenAI-compatible endpoints, screens resolved endpoint addresses
when DNS resolution is available, fails closed on unresolved live network
endpoints in CI and unresolved OTLP endpoints by default, repeats
OpenAI-compatible endpoint DNS screening before request dispatch, requires
explicit risk flags for non-interactive live configs that execute scripts, use
network egress, or pass host environment variables, bounds external-script
stdout/stderr capture, and expands recursive persisted-artifact redaction plus
preserved-field fail-closed checks for common secret token patterns. The
bundled fixture HMAC key is now accepted only for repository synthetic
examples; other fixture runs must provide an explicit key.
