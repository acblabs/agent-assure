...
This runbook covers the Python package upload path for `agent-assure` v0.6.5.
...
directory before package upload. For the v0.6.5 package release, the active
schema is `0.6.5` and the candidate schema directory is `schemas/v0.6.5` until
...
agent-assure schema export --out "${schema_review_dir}/v0.6.5"
git diff --no-index -- schemas/v0.6.5 "${schema_review_dir}/v0.6.5"
make schema-check
make release-check
python scripts/check_version_matches_tag.py v0.6.5
...
$SchemaReview = Join-Path $SchemaReviewRoot "v0.6.5"
agent-assure schema export --out $SchemaReview
git diff --no-index -- schemas/v0.6.5 $SchemaReview
make schema-check
make release-check
python scripts/check_version_matches_tag.py v0.6.5
...
`git diff -- schemas/v0.6.5`, run `make schema-force-includes`, then rerun
...
a stable `0.6.5` introduction is correctly considered newer than `0.6.5rc2`.
...
`0.6.5rc1`, then `0.6.5rc2` if another candidate is needed.

1. Create a candidate ref whose package metadata already contains the unique
   candidate version, for example `project.version = "0.6.5rc1"` and
   `agent_assure.__version__ = "0.6.5rc1"`.
...
4. Run the `Publish to TestPyPI` workflow manually from that ref and set
   `expected-version` explicitly to the same value, for example `0.6.5rc1`.
   ...
   candidate branch or commit; do not create or push a `v0.6.5rcN` tag.
...
version to `0.6.5`, run `python scripts/update_golden.py --update-golden` again,
...
commit before creating the `v0.6.5` tag. RC-generated sensitivity goldens must
...
1. moves the release entries from `Unreleased` under a dated `## 0.6.5`
   changelog heading;
2. creates `docs/release_notes/v0.6.5.md` and adds it to `mkdocs.yml`;
...
if `v0.6.5` does not match `project.version = "0.6.5"` and
`agent_assure.__version__ = "0.6.5"`, if the active schema constants do not
match the mapped release schema version `0.6.5`, or if `schemas/v0.6.5` is
...
python -m pip install --upgrade pip
python -m pip install agent-assure==0.6.5
agent-assure --version
