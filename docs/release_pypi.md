...
This runbook covers the Python package upload path for `agent-assure` v0.6.2.
...
directory before package upload. For the v0.6.2 package release, the active
schema is `0.6.2` and the candidate schema directory is `schemas/v0.6.2` until
...
agent-assure schema export --out "${schema_review_dir}/v0.6.2"
git diff --no-index -- schemas/v0.6.2 "${schema_review_dir}/v0.6.2"
make schema-check
make release-check
python scripts/check_version_matches_tag.py v0.6.2
...
$SchemaReview = Join-Path $SchemaReviewRoot "v0.6.2"
agent-assure schema export --out $SchemaReview
git diff --no-index -- schemas/v0.6.2 $SchemaReview
make schema-check
make release-check
python scripts/check_version_matches_tag.py v0.6.2
...
`git diff -- schemas/v0.6.2`, run `make schema-force-includes`, then rerun
...
a stable `0.6.2` introduction is correctly considered newer than `0.6.2rc2`.
...
`0.6.2rc1`, then `0.6.2rc2` if another candidate is needed.

1. Create a candidate ref whose package metadata already contains the unique
   candidate version, for example `project.version = "0.6.2rc1"` and
   `agent_assure.__version__ = "0.6.2rc1"`.
2. Build and verify locally with `make release-check`.
3. Run the `Publish to TestPyPI` workflow manually from that ref and set
   `expected-version` explicitly to the same value, for example `0.6.2rc1`.
   ...
   candidate branch or commit; do not create or push a `v0.6.2rcN` tag.
...
version to `0.6.2` before creating the final `v0.6.2` tag.
...
if `v0.6.2` does not match `project.version = "0.6.2"` and
`agent_assure.__version__ = "0.6.2"`, if the active schema constants do not
match the mapped release schema version `0.6.2`, or if `schemas/v0.6.2` is
...
python -m pip install --upgrade pip
python -m pip install agent-assure==0.6.2
agent-assure --version
