...
| `signed-release-evidence` | Release evidence, SBOM, wheel, and source distribution blobs can be signed keylessly in GitHub Actions and verified against exact workflow identity and blob bytes; PyPI publishing verifies the downloaded release bundle signatures before replay and package staging, and replay also checks manifest-listed hashes when files are present. |
