...
| `live-trajectory-analysis` | Live trajectory analysis derives privacy-filtered observable state paths, canonical transition profiles, explicit history-dependent checks, sequence-invariant results, and operational event-process summaries as protocol-bound review evidence with `not_evaluated` gate state. |
