...
| `measured-usage-cost-delta` | Usage-bearing artifacts can report measured token, tool, retry, latency, pricing snapshot ID/digest, declared estimated cost, per-cost-observation, and integer basis-point delta facts separately from governance verdicts. |
...
| `assurance-evidence-descriptors` | Development-RFC descriptors bind mutation evidence to its subject, method, suite, gate/waiver/date configuration, prerequisites, limitations, dependencies, and invalidation triggers. |
| `deterministic-control-mutation` | The development-RFC mutation command applies and evaluates one deterministic fixture challenge and publishes a coherent, manifest-bound output generation. |
| `mutation-independence-classes` | Mutation artifacts persist independence class and provenance origin so first-party post-control evidence is not presented as independent red-team evidence. |
| `signed-release-evidence` | Release evidence, its human-readable packet, manifest, replay, release notes, SBOM, wheel, and source distribution can be signed keylessly in a tag-only minimal GitHub Actions OIDC job only after every downloaded signing input is byte-matched to an independent rebuild and promoted under a new artifact ID, then verified without OIDC before publication from verifier-promoted artifacts. |
...
| `live-stochastic-evaluation` | Live commands require a frozen protocol, run declared adapters including static JSONL, no-shell external-script, and standard-library-HTTP OpenAI-compatible adapters, produce protocol-bound repeated live RunSets, report cluster-aware pooled and cluster-mean rates with explicit interval-center metadata, account for exclusions and incomplete stops, compare paired or fixed-reference reports using protocol-declared methods with exploratory guardrails, report provider/model operational distributions, and are covered by synthetic calibration tests. |
| `live-advanced-statistics` | Optional advanced live endpoint plans bind confirmatory or exploratory statistical endpoints to the protocol digest, report rare-event upper bounds at the protocol confidence level, observed cluster-correlation summaries, Bonferroni multiplicity controls, deterministic integer-seeded resampling, and paired randomization tests only when structural pairing checks and declared prerequisites are met. Synthetic tests cover null behavior, boundary behavior, multiplicity failures, and prerequisite failures. |
| `live-drift-monitoring` | Cross-window live drift monitoring consumes ordered live evaluation reports, checks material and timestamp-order comparability, and reports trend, adjacent-step, separate serial-dependence, and EWMA governance-health or control-reliability diagnostics as exploratory review signals by default when prerequisites are met. Synthetic tests cover comparable, low-window, and timestamp-order cases. |
| `live-trajectory-analysis` | Live trajectory analysis derives privacy-filtered observable state paths, canonical transition profiles, explicit history-dependent checks, sequence-invariant results, and operational event-process summaries as protocol-bound review evidence with `not_evaluated` gate state. Synthetic tests cover transition invariants, retry burst signals, missing timestamps, and counted-event handling. |
