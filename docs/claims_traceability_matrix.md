...
| `live-advanced-statistics` | Optional advanced live endpoint plans bind confirmatory or exploratory statistical endpoints to the protocol digest, report rare-event upper bounds, observed cluster-correlation summaries, multiplicity controls, deterministic integer-seeded resampling, and paired randomization tests only when structural pairing checks and declared prerequisites are met. |
