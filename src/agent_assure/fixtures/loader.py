...
def load_compiled_suite(path: Path, *, expected_digest: str | None = None) -> CompiledSuite:
    # Imported lazily because validation's schema registry imports the evaluator,
    # which imports this module's digest helper.
    ...
    compiled = project_validated_artifact_payload(
        payload,
        CompiledSuite,
        kind="compiled-suite",
    )
