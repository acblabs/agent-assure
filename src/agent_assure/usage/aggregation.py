...
def compare_usage_summaries(
    ...
    if (
        ...
        if baseline.currency == candidate.currency:
            cost_delta = candidate.estimated_cost_microusd - baseline.estimated_cost_microusd
        else:
            limitations.append(
                "Declared estimated cost was not compared because currencies differ."
            )
    ...
    return UsageSummaryDelta(
        ...
        estimated_cost_microusd_delta=cost_delta,
        currency=currency,
...
def format_usage_delta(delta: UsageSummaryDelta) -> str:
    ...
    metric_parts = [
        _format_delta("total_tokens", delta.total_tokens_delta),
        _format_delta("tool_calls", delta.total_tool_calls_delta),
        _format_delta("retries", delta.total_retries_delta),
        _format_delta("latency_ms", delta.total_latency_ms_delta),
    ]
    ...
    if delta.estimated_cost_microusd_delta is not None:
        parts.append(
            ...
            f"{delta.estimated_cost_microusd_delta:+d} micro-USD {delta.currency}"
        )
...
def _summary_from_summaries(summaries: tuple[UsageSummary, ...]) -> UsageSummary:
    ...
    cost, currency, cost_limitations = _sum_summary_cost(summaries)
...
def _sum_summary_cost(summaries: tuple[UsageSummary, ...]) -> tuple[int | None, str, list[str]]:
    cost_summaries = tuple(
        summary for summary in summaries if summary.estimated_cost_microusd is not None
    )
    if not cost_summaries:
        return None, summaries[0].currency if summaries else "USD", []
    ...
    return sum(summary.estimated_cost_microusd or 0 for summary in cost_summaries), currency, []
...
def _format_delta(label: str, value: int | None) -> str | None:
    if value is None:
        return None
