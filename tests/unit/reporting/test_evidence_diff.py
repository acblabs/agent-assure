...
def test_evidence_diff_marks_missing_candidate_case_as_changed() -> None:
    ...
    html = render_evidence_diff_html(
        baseline=baseline,
        candidate=candidate,
        baseline_summary=EvaluationSummary(runset_id="baseline", state=GateState.pass_),
        candidate_summary=EvaluationSummary(runset_id="candidate", state=GateState.pass_),
